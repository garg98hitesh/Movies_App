//Type rcc for class component and type rcfe for functional components
...
export default class Movies extends Component {
  constructor() {
    super();
    let favArr = JSON.parse(localStorage.getItem("movies-app") || "[]");
    favArr = favArr.map((m)=> m.id);
    this.state = {
      hover: "",
      parr: [1],
      currPage: 1,
  ...
  async componentDidMount() {
    //Side effects work that needs to be called in async and await to be called here and API calls to be made here
  ...
  handleRight = () => {
    ...
    this.setState({  //Remeber that setState function is an async function 
      parr: [...temparr],
  ...
  oldFavArr = JSON.parse(localStorage.getItem("movies-app") || "[]");
